 ...
 */
Ext.define('App.ux.data.proxy.IndexedDB', {
    ...
    config: {
         ...
         * Name of object store: Will default to the string after the last '.' in the model's class name
         */
    ...
    //
    //     //TODO: VK - EXTRA remove
    //     console.log("constructor");
    //
    //     // Ext.on("indexeddb-onupgradeneeded", this.checkDbSchema, this, { single: true });
    // },
    ...
    updateModel: function(model) {
        ...
        var modelName;
        var dbName = this.getDbName() || this.getObjectStoreName();

        function getDbName() {
            modelName = model.prototype.entityName;
            dbName = dbName || modelName.slice(modelName.lastIndexOf('.') + 1);
        ...
        if (model) {
            if (!this.getDbName()) {
                this.setDbName(getDbName());
            }
            if (!this.getObjectStoreName()) {
                this.setObjectStoreName(getDbName());
            }
    ...
    ensureDb: function(options, callback, scope) {
        ...
        //     Ext.fireEvent("indexeddb-onupgradeneeded", request);
        // };
    },
    ...
    openDbSuccess: function (request, callback, scope) {
        ...
        Ext.each(this.getIndices(), function (data) {
    ...
    // checkDbSchema: function(request) {
    //     var me = this,
    //         db = request.result,
    //         sStoreName = me.getObjectStoreName();
    //
    //     if (!db.objectStoreNames.contains(sStoreName)) {
    //         me.openDbSetSchema(request);
    //     }
    // },

    openDbSetSchema: function(request) {
        var store = request.result.createObjectStore(this.getObjectStoreName(), {keyPath: this.idProp});
        var i;
        var ln;
        ...
        for (i = 0, ln = indices.length; i < ln; i += 1) {
            ...
            if (Ext.isObject(index)) {
                store.createIndex(index.name, index.field || index.name, index.options || {unique: false});
            } else {
    ...
    getDbTxWithDb: function(type, options, callback, scope) {
        var db = this.db;
        var sStoreName = this.getObjectStoreName();

        var tx = db.transaction([sStoreName], type);
    ...
    createTransaction: function(tx) {
        ...
        Ext.apply(options, {
            tx: tx,
            object_store: tx.objectStore(this.getObjectStoreName()),
